...
RSpec.describe(Income, type: :model) do
  ...
  describe "validations" do
    ...
    it "requires transaction_category" do
      income = described_class.new(unit_value: 100.0, date: Time.zone.today)
      expect(income).not_to(be_valid)
      expect(income.errors[:transaction_category]).to(include("must exist"))
    end

    it "requires unit_value" do
      income = described_class.new(transaction_category: transaction_categories(:daily_transaction),
    ...
    it "requires date" do
      income = described_class.new(transaction_category: transaction_categories(:daily_transaction), unit_value: 100.0)
      expect(income).not_to(be_valid)
  ...
  describe "fixtures" do
    ...
    it "has different transaction categories" do
      categories = described_class.joins(:transaction_category).pluck("transaction_categories.name")
