...
RSpec.describe(Athlete, type: :model) do
  ...
  describe "validations" do
    ...
    it "requires name" do
      athlete = described_class.new(phone: 1_234_567_890, date_of_birth: Time.zone.today)
      expect(athlete).not_to(be_valid)
      expect(athlete.errors[:name]).to(include("can't be blank"))
    end

    it "requires phone" do
      athlete = described_class.new(name: "Test Athlete", date_of_birth: Time.zone.today)
      expect(athlete).not_to(be_valid)
      expect(athlete.errors[:phone]).to(include("can't be blank"))
    end

    it "requires date_of_birth" do
      athlete = described_class.new(name: "Test Athlete", phone: 1_234_567_890)
      expect(athlete).not_to(be_valid)
  ...
  describe "attributes" do
    ...
    it "has the correct phone" do
      expect(athlete.phone).to(eq(1_234_567_890))
    end
  ...
  describe "fixtures" do
    ...
    it "has different phone numbers" do
      phones = described_class.pluck(:phone)
  ...
  describe "data types" do
    ...
    it "stores phone as integer" do
      ...
      expect(athlete.phone).to(eq(9_876_543_210))
    end
  ...
  describe "model behavior" do
    it "can create a new athlete" do
      athlete = described_class.new(name: "New Athlete", phone: 1_112_223_333, date_of_birth: Time.zone.today)
    ...
    it "can update an existing athlete" do
      ...
      athlete.phone = 9_998_887_777
      expect(athlete.save).to(be(true))
      expect(athlete.reload.phone).to(eq(9_998_887_777))
    end
  end
  ...
  describe "associations" do
    describe "has_many :matches, through: :athlete_matches" do
      ...
      it "returns empty collection for athlete with no matches" do
        athlete = described_class.create!(name: "New Athlete", phone: 1_112_223_333, date_of_birth: Time.zone.today)
